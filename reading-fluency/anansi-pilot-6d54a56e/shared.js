(() => {
  ...
  const phonologicalAwareness = {
    ...
    sentence: "high", paragraph: "mid", worksheet: "high"
  };
  const spellingAwareness = {
    cover: "none", vocab: "mid", game2: "mid", game1: "low",
    sentence: "mid", paragraph: "mid", worksheet: "high"
  };
  ...
  const trendGroupIds = {
    ...
    sentence: "trend.fluency", paragraph: "trend.fluency"
  };
  ...
  function renderWordFind() {
    ...
    return `<div class="activity-view">${pageHead("이 책에 나올까요?", "낱말을 하나씩 살펴봐요")}
      <div class="word-quiz-layout">
        <div class="scene-card"><img src="${(lesson.sentences || [])[0] ? `assets/book/page-${String(lesson.sentences[0].page).padStart(2, "0")}.webp` : lesson.coverImage}" alt="그림책 《${esc(pack.bookTitle)}》에 나오는 친구들"></div>
  ...
  function activityCounts(id) {
    ...
    if (id === "game1") {
      /* 문장 완성은 **회차마다 다른 모듈이 그린다.** 진행을 내는 통로를 골라 쓴다.
       *   · activity2-sequence 가 그리는 회차 → ONQ_GAME1_QA (참 문항 수는 itemCount)
       *   · activity-overrides 가 그리는 회차 → ONQ_GAME1_PROGRESS
       * 여기서 lesson.game1.length 를 그냥 쓰면 아이 화면이 「1 / 6」인데
       * 코치에게는 「0 / 15」로 보인다 — 실제로 그랬다. */
      const qa = window.ONQ_GAME1_QA;
      const live = qa?.getState?.() || window.ONQ_GAME1_PROGRESS?.getState?.();
      const total = Number.isFinite(live?.total) ? live.total
                  : Number.isFinite(qa?.itemCount) ? qa.itemCount
                  : lesson.game1.length;
