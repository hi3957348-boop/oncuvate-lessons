(() => {
  ...
  const g1 = { index: 0, cleared: 0, selected: [], fails: 0, startedAt: performance.now(), timerOn: false, seconds: 30, timer: null };

  /* 코치에게 문장 완성 진행을 알리는 통로.
   *
   * 🔴 `ONQ_GAME1_QA` 라는 이름은 쓰지 마라 — 아래 normalizePages 에서 그 이름은
   *    「바깥 도구가 이 화면을 그리고 있다」는 표시로 쓰인다. 그 이름으로 내면
   *    이 파일이 그리던 화면이 꺼지고 셸의 옛 낱말 조각 판이 대신 뜬다.
   *
   * `g1.index` 로는 셀 수 없다 — 마지막 문항을 마치면 0으로 돌아간다(아래 % 연산).
   * 그래서 마친 개수를 따로 센다. 안 그러면 코치 콘솔이 계속 0 으로 보인다. */
  window.ONQ_GAME1_PROGRESS = {
    getState: () => ({ index: g1.cleared, total: lesson.game1.length, completed: g1.cleared >= lesson.game1.length })
  };
  ...
  function choosePiece(button, value) {
    ...
    if (g1.selected.length === item.chunks.length) {
      setTimeout(() => {
        // 마친 개수는 따로 센다 — 아래 index 는 한 바퀴 돌면 0 으로 되돌아간다.
        if (g1.cleared < lesson.game1.length) g1.cleared += 1;
