(() => {
  ...
  function printDeck() {
    ...
    return `<section class="ws-print-deck" aria-hidden="true">${["A", "B", "C"].map((stage, page) => `<section class="ws-print-page">
      <header class="ws-print-head"><div><span>${esc(window.ONQ_CONTENT_PACK?.bookTitle || "")} · ${esc(window.ONQ_CONTENT_PACK?.sessions?.[sessionKey]?.sessionLabel || "")}</span><h1>${stage}. ${esc(stageMeta[stage].title)}</h1><p>${esc(stageMeta[stage].help)}</p></div><div class="ws-print-name">이름 <span></span></div></header>
