(() => {
  ...
  // 낱말 소리 — 켜면 낱말이 바뀔 때 저절로 들려준다. 꺼도 「다시 듣기」는 살아 있다.
  const SOUND_STORE = "onq.expansion.sound.v1";
  let soundOn = true;
  try { soundOn = localStorage.getItem(SOUND_STORE) !== "off"; } catch (_) {}
  ...
  function head(kicker) {
    const sound = `<span class="onq-expansion-sound sentence-segmented" role="group" aria-label="낱말 소리"><button type="button" data-expansion-action="sound-on" aria-pressed="${soundOn}">ON</button><button type="button" data-expansion-action="sound-off" aria-pressed="${!soundOn}">OFF</button></span>`;
    return `<div class="onq-expansion-head"><span class="onq-expansion-kicker">${esc(kicker)}</span><span class="onq-expansion-sound-label">낱말 소리</span>${sound}<button class="onq-expansion-close" type="button" data-expansion-action="close" aria-label="연습 닫기">×</button></div><p class="onq-expansion-source">${esc(practice.row.target)}와 같은 소리를 연습해요</p>`;
  }
  ...
  function markup() {
    ...
    if (practice.phase === "read") {
      ...
      const fallback = practice.status || (assessed
        ? "낱말을 차례대로 소리 내어 읽어요. 읽는 소리를 살펴보려면 ‘읽기 시작’, 혼자 읽고 넘어가려면 오른쪽을 눌러요."
        : "낱말을 차례대로 읽고 아래를 눌러요.");
      return `${head("낱말 모아 읽기")}${list}<p class="onq-expansion-status" role="status">${esc(fallback)}</p><div class="onq-expansion-actions">${next}</div>${practice.recording && window.ONQ_REC_INDICATOR ? window.ONQ_REC_INDICATOR("듣고 있어요 — 낱말을 차례대로") : ""}`;
    }
  ...
  async function armCurrent() {
    ...
    try {
      ...
      practice.status = "낱말을 차례대로 읽어요. 다 읽으면 아래를 눌러요.";
      renderPanel();
    } catch (_) {
      if (!practice) return;
  ...
  function playCurrent(byRequest) {
    if (!practice || practice.phase !== "learn") return;
    // 꺼져 있으면 저절로는 안 들려준다. 청해서 누른 것(byRequest)은 들려준다.
    if (!soundOn && !byRequest) {
      practice.status = "읽어 보고 ‘다음’을 눌러요. 소리가 필요하면 스피커를 눌러요.";
      renderPanel();
      return;
    }
  ...
  async function finishCurrent() {
    ...
    practice.status = "읽은 낱말을 한 번에 살펴보고 있어요.";
    renderPanel();
    let result = null;
  ...
  function applyResult(result) {
    ...
    // 첨삭은 낱말 전체에 한 번 — 어디가 달랐는지는 표시로 말한다(점수·판정 없음).
  ...
  document.addEventListener("click", event => {
    ...
    else if (action === "audio") { window.ONQ_PACING?.replay(`exp:${practice.row.id}:${practice.index}`); playCurrent(true); }
    else if (action === "sound-on" || action === "sound-off") {
      soundOn = action === "sound-on";
      try { localStorage.setItem(SOUND_STORE, soundOn ? "on" : "off"); } catch (_) {}
      renderPanel();
      if (soundOn && practice.phase === "learn") playCurrent(true);
    }
