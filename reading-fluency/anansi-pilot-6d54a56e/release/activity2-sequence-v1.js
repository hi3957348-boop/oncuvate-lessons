(() => {
  ...
  // 문장은 **content-pack에서 가져온다.** 예전에는 회차별 문장을 여기에 박아 두고
  // 모르는 회차는 session02로 물러났는데, 그러면 **다른 책 문장이 그대로 떴다**
  // (박혁거세 3회차에 「머리가 가렵지만 긁을 수가 없어요」가 나왔다).
  //
  // 이 활동은 낱말 카드를 순서대로 고르는 것이라 **3~5어절 문장에서만** 쓸모가 있다.
  // 본문에 그런 문장이 넉넉하지 않으면 조용히 물러나고, 글자 조각 방식이 대신 뜬다.
  const source = [
    ...(lesson.sentences || []).map(item => item.text),
    ...(lesson.sentences || []).map(item => item.apply)
  ].filter(Boolean);

  const seen = new Set();
  const items = source
    .map(text => ({ text, words: text.replace(/[.!?]+$/u, "").split(/\s+/) }))
    .filter(item => item.words.length >= 3 && item.words.length <= 5)
    .filter(item => !seen.has(item.text) && seen.add(item.text))
    .slice(0, 6)
    .map((item, index) => ({ id: `sequence-${sessionKey}-${index + 1}`, text: item.text, words: item.words }));
  if (items.length < 4) return;   // 넉넉하지 않으면 이 활동을 쓰지 않는다
