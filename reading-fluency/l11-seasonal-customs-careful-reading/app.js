(() => {
  ...
  const contentId = "l11-seasonal-customs-careful-reading";
  ...
  const helpMessages = [
    ...
    "절기 이름과 계절 단서를 확인해 알맞은 계절 칸에 놓아요.",
  ...
  document.querySelectorAll("[data-predict]").forEach(button => {
    button.addEventListener("click", () => {
      ...
      button.dataset.correct = button.dataset.predict.startsWith("계절") ? "true" : "false";
      emit("answer", { activityId: "predict-seasonal-customs", itemId: "prediction", value: state.prediction });
    });
  });
  ...
  const firstQuestions = [
    {
      question: "세시 풍속은 무엇을 뜻하나요?",
      options: ["날씨를 미리 알려 주는 방법", "계절에 맞추어 이어 온 생활 습관", "명절에만 먹는 특별한 음식"],
      correct: 1,
      cueOptions: ["세시 풍속", "무엇을", "뜻하나요"],
      cueCorrect: 0,
      explanation: "세시 풍속은 한 해의 절기나 달, 계절에 맞추어 이어 온 생활 습관이에요."
    },
    {
      question: "입춘에 한 일로 알맞은 것은 무엇인가요?",
      options: ["콩을 뿌리거나 좋은 글귀를 붙였다.", "팥죽을 쑤어 이웃과 나누었다.", "창포물에 머리를 감았다."],
      correct: 0,
      cueOptions: ["입춘에", "한 일", "알맞은 것"],
      cueCorrect: 0,
      explanation: "봄이 시작되는 입춘에는 콩을 뿌리거나 대문에 좋은 글귀를 붙였어요."
    },
    {
      question: "동지에 우리 조상들이 한 일은 무엇인가요?",
      options: ["보리싹을 보고 농사를 짐작했다.", "옷과 이불을 햇볕에 말렸다.", "팥죽을 쑤어 이웃과 나누어 먹었다."],
      correct: 2,
      cueOptions: ["동지에", "우리 조상들이", "한 일"],
      cueCorrect: 0,
      explanation: "밤이 가장 긴 동지에는 팥죽을 쑤어 이웃과 나누어 먹었어요."
    }
  ];
  ...
  // 4. 계절 달력 복구 — 절기 풍속을 알맞은 계절에 배치
  const pipelineTasks = [
    { id: "ipchun", stage: "spring", cue: "입춘", text: "입춘에는 콩을 뿌리거나 대문에 좋은 글귀를 붙였습니다." },
    { id: "mangjong", stage: "summer", cue: "망종", text: "망종에는 보리나 벼의 씨를 뿌렸습니다." },
    { id: "cheoseo", stage: "autumn", cue: "처서", text: "처서에는 벌초하고 옷과 이불을 햇볕에 말렸습니다." },
    { id: "ipdong", stage: "winter", cue: "입동", text: "입동에는 시루떡을 나누고 김장을 준비했습니다." },
    { id: "gyeongchip", stage: "spring", cue: "경칩", text: "경칩에는 보리싹을 보고 그해 농사를 짐작했습니다." },
    { id: "haji", stage: "summer", cue: "하지", text: "하지 무렵까지 비가 오지 않으면 기우제를 지냈습니다." },
    { id: "hallo", stage: "autumn", cue: "한로", text: "한로 무렵에는 곡식을 거두느라 바빴습니다." },
    { id: "dongji", stage: "winter", cue: "동지", text: "동지에는 팥죽을 쑤어 이웃과 나누어 먹었습니다." }
  ];
  const pipelineStageMeta = {
    spring: { label: "봄", total: 2 },
    summer: { label: "여름", total: 2 },
    autumn: { label: "가을", total: 2 },
    winter: { label: "겨울", total: 2 }
  };
  const pipelineStageOrder = ["spring", "summer", "autumn", "winter"];
  ...
  const pipelineStageCounts = { spring: 0, summer: 0, autumn: 0, winter: 0 };
  ...
  function renderPipelineTask() {
    ...
    pipelineFeedback.textContent = "절기 이름과 계절 단서를 확인해 보세요.";
  ...
  function placePipelineCard(stage, interaction = "tap") {
    ...
    emit("answer", {
      activityId: "restore-season-calendar",
    ...
    if (!correct) {
      ...
      if (pipelineAttempts === 1) {
        pipelineFeedback.textContent = "아직 맞지 않아요. 절기 이름과 계절을 다시 연결해 봐요.";
      } else {
        renderPipelineCardText(task, true);
        pipelineFeedback.textContent = "‘" + task.cue + "’가 어느 계절의 절기인지 확인해 보세요.";
      }
    ...
    pipelineFeedback.textContent = (pipelineAttempts > 1 ? "다시 확인해 연결했어요. " : "정확해요. ") + "‘" + task.cue + "’ 절기는 " + pipelineStageMeta[task.stage].label + "이에요.";
  ...
  const conditionAnswers = { subject: "ancestors", time: "ipdong", range: "all", evidence: "two" };
  ...
  verifyButton.addEventListener("click", () => {
    ...
    const answerCorrect = selected.length === 2 && selected[0] === "kimchi" && selected[1] === "ricecake";
