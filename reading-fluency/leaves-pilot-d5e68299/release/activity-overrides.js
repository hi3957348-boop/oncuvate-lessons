(() => {
  ...
  // 젤리캡쳐 5×5 판 — 회차마다 **그 회차 본문 어절**로만 채운다.
  // 한 줄이 한 문장에서 나오므로 줄을 소리 내어 읽어도 글이 이어진다.
  const BOARDS = {
    session01: [
      ["가을이","되면","나뭇잎의","색이","달라져요"],
      ["초록빛이","줄어들고","노란빛이나","붉은빛이","나타나요"],
      ["그러다가","잎이","나뭇가지에서","떨어져요","이렇게"],
      ["나무에서","떨어진","잎을","낙엽이라고","해요"],
      ["가을이","깊어지면","날씨가","점점","추워져요"]
    ],
    session02: [
      ["겨울에는","땅이","얼어","물을","얻기"],
      ["어떤","잎에서","빠져나가는","줄이려고","떨어뜨려요"],
      ["하지만","모든","떨어뜨리는","것은","아니에요"],
      ["잎이","뾰족한","소나무는","한겨울에도","지녀요"],
      ["나무들은","모습으로","겨울을","준비를","한답니다"]
    ]
  };
  const rows = BOARDS[sessionKey] || BOARDS.session01;
