(() => {
  ...
  // 한 문장에 빈칸이 여럿일 수 있다. `target` 은 글자열 하나이거나 여럿의 배열이다.
  // 배열이 아니면 하나짜리 배열로 감싸 **아래 길을 하나로** 만든다 — 옛 자료는 동작이 그대로다.
  function targetsOf(item) {
    return Array.isArray(item.target) ? item.target : [item.target];
  }
  // 슬롯마다 보기가 다를 수 있다. `choices` 가 배열의 배열이면 슬롯별, 아니면 모두 같은 보기.
  function choicesFor(item, slot) {
    const c = item.choices || [];
    return Array.isArray(c[0]) ? (c[slot] || []) : c;
  }
  // 빈칸 사이의 글 조각들. 빈칸이 n개면 조각은 n+1개다.
  function splitSentence(sentence, target) {
    const targets = Array.isArray(target) ? target : [target];
    const parts = []; let rest = sentence;
    for (const one of targets) {
      const index = rest.indexOf(one);
      if (index < 0) { parts.push(rest); rest = ""; continue; }
      parts.push(rest.slice(0, index));
      rest = rest.slice(index + one.length);
    }
    parts.push(rest);
    return parts;
  }
  ...
  // 답은 **슬롯마다** 따로 담는다. 옛 자료가 남긴 글자열 하나는 첫 슬롯으로 옮겨 읽는다.
  function answersOf(item) {
    const raw = state.answers[state.pack][state.stage][item.id];
    const list = Array.isArray(raw) ? raw.slice() : (raw ? [raw] : []);
    while (list.length < targetsOf(item).length) list.push("");
    return list;
  }
  function answerAt(item, slot) {
    return answersOf(item)[slot] || "";
  }
  function setAnswerAt(item, slot, value) {
    const list = answersOf(item);
    list[slot] = value;
    state.answers[state.pack][state.stage][item.id] = list;
    return list;
  }
  // 화면 밖(기록·발화)에서 한 덩이로 볼 때 쓴다.
  function answerFor(item) {
    return answersOf(item).join(" ");
  }

  function isCorrect(item) {
    const targets = targetsOf(item), list = answersOf(item);
    return targets.every((one, i) => normalize(list[i]) === normalize(one));
  }
  ...
  function recordAnswer(item, response, source, finalAttempt = true) {
    if (Array.isArray(response)) state.answers[state.pack][state.stage][item.id] = response.slice();
    else state.answers[state.pack][state.stage][item.id] = [response];
  ...
  function sentenceRow(item, number, stage = state.stage, print = false) {
    ...
    const parts = splitSentence(item.sentence, item.target);
    const answers = answersOf(item);
    const correct = isCorrect(item);
    const status = answers.some(Boolean) ? (correct ? "is-correct" : "is-pending") : "";
    const slotLabel = targets.length > 1 ? ["첫째", "둘째", "셋째", "넷째"] : null;
    // 빈칸 하나를 그린다. 여러 개면 이 함수가 슬롯 수만큼 불린다.
    const blankFor = slot => {
      const one = targets[slot];
      const answer = answers[slot] || "";
      const ok = normalize(answer) === normalize(one);
      const hint = stage === "B" ? `<span class="ws-initial" aria-label="첫소리 힌트">${esc(initialSounds(one))}</span>` : "";
      if (print) return `<span class="ws-print-blank ${stage === "B" ? "with-hint" : ""}">${stage === "B" ? esc(initialSounds(one)) : ""}</span>`;
      if (stage === "A") return `<span class="ws-blank ${ok ? "filled" : ""}">${ok ? esc(one) : "빈칸"}</span>`;
      return `<span class="ws-input-wrap">${hint}<input class="ws-input" type="text" inputmode="text" autocomplete="off" spellcheck="false" value="${esc(answer)}" data-ws-input="${esc(item.id)}" data-ws-slot="${slot}" aria-label="${number}번 ${slotLabel ? slotLabel[slot] + " " : ""}빈칸에 들어갈 낱말" style="--ws-chars:${Math.max(4, one.length)}"></span>`;
    };
    // 조각과 빈칸을 번갈아 잇는다. 확인 단추는 줄 끝에 하나만 둔다.
    const line = parts.map((seg, i) => esc(seg) + (i < targets.length ? blankFor(i) : "")).join("");
    const checkBtn = (!print && stage !== "A")
      ? `<button class="ws-check" type="button" data-ws-action="check" data-ws-item="${esc(item.id)}">확인</button>` : "";
    const choices = stage === "A" && !print
      ? targets.map((one, slot) => {
          const answer = answers[slot] || "";
          const ok = normalize(answer) === normalize(one);
          return `<div class="ws-choices" aria-label="${number}번 ${slotLabel ? slotLabel[slot] + " 빈칸 " : ""}보기">${
            slotLabel ? `<span class="ws-slot-tag">${slotLabel[slot]}</span>` : ""}${
            choicesFor(item, slot).map(choice => `<button type="button" class="ws-choice ${answer === choice ? (ok ? "selected correct" : "selected") : ""}" data-ws-action="choose" data-ws-item="${esc(item.id)}" data-ws-slot="${slot}" data-ws-value="${esc(choice)}">${esc(choice)}</button>`).join("")}</div>`;
        }).join("")
      : "";
    ...
    return `<article class="ws-item ${status}" data-ws-row="${esc(item.id)}">
      ...
      <div class="ws-item-body">
        <div class="ws-sentence">${line}${checkBtn}</div>
        ${choices}
  ...
  function printDeck() {
    ...
    return `<section class="ws-print-deck" aria-hidden="true">${["A", "B", "C"].map((stage, page) => `<section class="ws-print-page">
      <header class="ws-print-head"><div><span>${esc(window.ONQ_CONTENT_PACK?.bookTitle || "")}${Object.keys(window.ONQ_CONTENT_PACK?.sessions || {}).length > 1 ? " · " + esc(window.ONQ_CONTENT_PACK.sessions[sessionKey].sessionLabel) : ""}</span><h1>${stage}. ${esc(stageMeta[stage].title)}</h1><p>${esc(stageMeta[stage].help)}</p></div><div class="ws-print-name">이름 <span></span></div></header>
      <div class="ws-print-list">${pack.items.map((item, index) => sentenceRow(item, index + 1, stage, true)).join("")}</div>
      ${stage === "A" ? `<footer class="ws-print-bank"><strong>보기</strong>${pack.items.flatMap(item => targetsOf(item)).map(one => `<span>${esc(one)}</span>`).join("")}</footer>` : ""}
  ...
  document.addEventListener("click", event => {
    ...
    if (action === "choose") {
      const slot = Number(button.dataset.wsSlot || 0);
      recordAnswer(item, setAnswerAt(item, slot, button.dataset.wsValue), "choice", true);
      rerender();
      return;
    }
    if (action === "check") {
      // 한 줄에 빈칸이 여럿이면 **다 모아** 한 번에 본다.
      const inputs = [...document.querySelectorAll(`[data-ws-input="${CSS.escape(item.id)}"]`)];
      const list = targetsOf(item).map((_, i) => {
        const box = inputs.find(node => Number(node.dataset.wsSlot || 0) === i);
        return box ? box.value : "";
      });
      recordAnswer(item, list, "typed", true);
      rerender();
    }
  }, true);
  ...
  document.addEventListener("input", event => {
    ...
    if (!item) return;
    const slot = Number(input.dataset.wsSlot || 0);
    const list = setAnswerAt(item, slot, input.value);
    if (isCorrect(item)) {
      recordAnswer(item, list, "typed_auto", false);
