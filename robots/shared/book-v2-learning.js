(function () {
  ...
  var USE_JELLY_ACTIVITY_ICONS = BOOK_CONFIG.useJellyActivityIcons === true
    || /(?:^|\/)fingernail-fieldmouse\/(?:index\.html)?$/.test(location.pathname);
  ...
  function itemActivityId(item, suffix) {
    var lessonId = (BOOK_CONFIG.meta && BOOK_CONFIG.meta.lessonId) || BOOK_CONFIG.id || "lesson";
    var base = item && item.activityId
      ? item.activityId
      : [lessonId, state.section, state.activity, String(state.index + 1).padStart(3, "0")].join(":");
    return suffix ? base + ":" + suffix : base;
  }
  function trackingAttributes(item, kind, choice, suffix) {
    var attributes = ' data-track="' + escapeHtml(kind) + '" data-track-manual="true" data-activity-id="' +
      escapeHtml(itemActivityId(item, suffix)) + '"';
    if (choice !== undefined && choice !== null) attributes += ' data-choice="' + escapeHtml(choice) + '"';
    return attributes;
  }
  function emitTracking(type, item, detail, suffix) {
    emitTrackingById(type, itemActivityId(item, suffix), detail);
  }
  function emitTrackingById(type, activityId, detail) {
    var payload = Object.assign({ type: type, activityId: activityId }, detail || {});
    window.dispatchEvent(new CustomEvent("oncuvate:track", { detail: payload }));
  }
  function worksheetGroupId(type) {
    var lessonId = (BOOK_CONFIG.meta && BOOK_CONFIG.meta.lessonId) || BOOK_CONFIG.id || "lesson";
    return lessonId + ":worksheet:" + type;
  }
  function worksheetItemId(item, type, index) {
    var base = item && item.activityId
      ? item.activityId
      : worksheetGroupId("setPool") + ":" + String(index + 1).padStart(3, "0");
    return base + ":" + type;
  }
  ...
  function isLocalStudentPreview() {
    return !!(window.OcSession && window.OcSession.role === "student" && window.OcSession.localPreview);
  }
  function isTeacher() {
  ...
  function roleName() {
    if (isLocalStudentPreview()) return "학생 미리보기";
  ...
  function buildLearningPanel(panel, section) {
    ...
    panel.addEventListener("click", function (event) {
      ...
      if (activityButton) {
        if (!isTeacher() && !isLocalStudentPreview()) return;
        ...
        var studentMayChange = isLocalStudentPreview() || (name === "difficulty" && pacing(section) === "student");
  ...
  function buildWorksheet(panel) {
    ...
    panel.addEventListener("input", function (event) {
      var trackedInput = event.target.closest('[data-track="answer"]');
      if (trackedInput) trackedInput.dataset.choice = trackedInput.value || "";
    });
  ...
  function renderWorksheet(type) {
    ...
    if (type === "josa") {
      var rows = sharedSet
        ? sharedSet.map(function (item, index) {
            return { prefix: item.prefix, rest: item.rest, particle: item.particle, activityId: worksheetItemId(item, "josa", index) };
          })
        : WORKSHEET_ACTIVITIES.josaPool && WORKSHEET_ACTIVITIES.josaPool.length
        ? shuffle(WORKSHEET_ACTIVITIES.josaPool).slice(0, WORKSHEET_ACTIVITIES.josaCount || 5).map(function (row, index) {
            return { prefix: row[0], rest: row[1], particle: row[2], activityId: worksheetGroupId("josa") + ":" + String(index + 1).padStart(3, "0") };
          })
      ...
      sheet.innerHTML = worksheetHeader("조사 채우기", "빈칸에 알맞은 조사를 골라 문장을 완성해 보세요.") +
        '<ol class="oc-ws-list">' + rows.map(function (row, index) {
          var particles = josaChoices(row.particle);
          var options = particles.map(function (particle) {
            return '<option value="' + escapeHtml(particle) + '">' + escapeHtml(particle) + '</option>';
          }).join("");
          return '<li data-activity-id="' + escapeHtml(row.activityId) + '"><span>' + row.prefix + '</span><select data-answer="' + escapeHtml(row.particle) + '"' +
            trackingAttributes({ activityId: row.activityId }, "answer", "") + ' aria-label="' + (index + 1) + '번 문장 조사 선택"><option value="">고르기</option>' + options + '</select><span>' + row.rest + '</span><i></i></li>';
        }).join("") + '</ol><button class="oc-ws-check" type="button" data-ws-check="josa"' + trackingAttributes({ activityId: worksheetGroupId("josa") }, "activity-complete") + '>확인하기</button>';
    } else if (type === "dictation") {
      var dictation = sharedSet
        ? sharedSet.map(function (item, index) { return { text: item.sentence, activityId: worksheetItemId(item, "dictation", index) }; })
        : WORKSHEET_ACTIVITIES.dictationPool && WORKSHEET_ACTIVITIES.dictationPool.length
        ? shuffle(WORKSHEET_ACTIVITIES.dictationPool).slice(0, WORKSHEET_ACTIVITIES.dictationCount || 5).map(function (text, index) {
            return { text: text, activityId: worksheetGroupId("dictation") + ":" + String(index + 1).padStart(3, "0") };
          })
        : (WORKSHEET_ACTIVITIES.dictation || ["내과에 가요.", "목이 아파요.", "안과에서 진찰을 받아요.", "피부과에 가야 해요.", "발목을 삐끗했어요."]).map(function (text, index) {
            return { text: text, activityId: worksheetGroupId("dictation") + ":" + String(index + 1).padStart(3, "0") };
          });
      sheet.innerHTML = worksheetHeader("문장 받아쓰기", "소리를 듣고 문장을 천천히 적어 보세요.") +
        '<ol class="oc-ws-list oc-dictation">' + dictation.map(function (item, index) {
          return '<li data-activity-id="' + escapeHtml(item.activityId) + '"><button type="button" data-ws-play="' + escapeHtml(item.text) + '" aria-label="' + (index + 1) + '번 문장 소리 듣기">소리 듣기</button><input type="text" data-answer="' + escapeHtml(item.text.replace(/[.\s]/g, "")) + '"' + trackingAttributes({ activityId: item.activityId }, "answer", "") + ' aria-label="' + (index + 1) + '번 받아쓰기 답"><i></i></li>';
        }).join("") + '</ol><button class="oc-ws-check" type="button" data-ws-check="dictation"' + trackingAttributes({ activityId: worksheetGroupId("dictation") }, "activity-complete") + '>확인하기</button>';
    } else {
      var batchim = (sharedSet
        ? sharedSet.map(function (item, index) { return { answer: item.sentence, activityId: worksheetItemId(item, "batchim", index) }; })
        ...
        var item = typeof row === "string" ? { answer: row } : row;
        if (!item.activityId) item.activityId = worksheetGroupId("batchim") + ":" + String(index + 1).padStart(3, "0");
        return item;
      });
        ...
        '<ol class="oc-ws-list oc-batchim-list is-sentence">' + batchim.map(function (row, index) {
          ...
          var syllables = answerLetters.map(function (answerLetter, letterIndex) {
            ...
            return '<input type="text" maxlength="1" class="oc-syllable-input ' + (needsFix ? "needs-fix" : "is-given") + '" value="' +
              escapeHtml(shownLetter) + '" data-syllable-answer="' + escapeHtml(answerLetter) + '"' + trackingAttributes({ activityId: row.activityId }, "answer", shownLetter) + ' ' +
              (needsFix ? "" : "readonly ") + 'aria-label="' + (letterIndex + 1) + '번째 음절">';
          }).join("");
          return '<li data-activity-id="' + escapeHtml(row.activityId) + '"><span class="oc-batchim-number">' + (index + 1) + '</span><button type="button" class="oc-batchim-listen" data-ws-play="' + escapeHtml(row.answer) + '" aria-label="' + (index + 1) + '번 문장 듣기"><b></b>문장 듣기</button>' +
            '<div class="oc-syllable-editor" aria-label="받침이 빠진 문장">' + syllables + '</div><i class="oc-sentence-mark"></i></li>';
        }).join("") + '</ol><button class="oc-ws-check" type="button" data-ws-check="batchim"' + trackingAttributes({ activityId: worksheetGroupId("batchim") }, "activity-complete") + '>확인하기</button>';
    }
  }
  ...
  function checkWorksheet() {
    refs.worksheet.querySelectorAll("[data-answer]").forEach(function (input) {
      ...
      emitTrackingById("answer", activityId, { choice: input.value || "", correct: correct });
      emitTrackingById("activity-complete", activityId, { choice: input.value || "", correct: correct });
    });
    refs.worksheet.querySelectorAll(".oc-batchim-list.is-sentence > li").forEach(function (row) {
      ...
      emitTrackingById("answer", row.dataset.activityId, { choice: combinedChoice, correct: allCorrect });
      emitTrackingById("activity-complete", row.dataset.activityId, { choice: combinedChoice, correct: allCorrect });
    });
  }
  ...
  function nextQuestion() {
    ...
    if (!isTeacher() && !isLocalStudentPreview() && pacing() !== "student") return;
  ...
  function buildFindTextMarkup(highlightStart, highlightLength, centerSentenceIndex, selectedIndices) {
    ...
    container.querySelectorAll(".word").forEach(function (wordEl) {
      ...
      wordEl.setAttribute("data-track", "answer");
      wordEl.setAttribute("data-track-manual", "true");
      wordEl.setAttribute("data-activity-id", itemActivityId(currentItem()));
      wordEl.setAttribute("data-choice", String(w));
  ...
  function submitLengthGuess(count) {
    ...
    var item = currentItem();
    var correct = count === item.parts.length;
    if (correct) {
    ...
    emitTracking("answer", item, { choice: String(count), correct: correct }, "length");
    if (correct) emitTracking("activity-complete", item, { choice: String(count), correct: true }, "length");
    renderLearning();
  }
  ...
  function submitAnswer(value) {
    ...
    var trackedChoice = Array.isArray(value) ? value.join("|") : String(value == null ? "" : value);
    emitTracking("answer", item, { choice: trackedChoice, correct: correct, attemptNumber: state.attempts });
    emitTracking("activity-complete", item, { choice: trackedChoice, correct: correct, attemptNumber: state.attempts });
  ...
  function renderLearning() {
    ...
    panel.querySelectorAll("[data-activity]").forEach(function (button) {
      ...
      button.disabled = !isTeacher() && !isLocalStudentPreview();
    });
    panel.querySelectorAll("[data-option] button").forEach(function (button) {
      ...
      var studentMayChange = isLocalStudentPreview() || (name === "difficulty" && pacing(state.section) === "student");
  ...
  function renderIntro(session) {
    var activity = currentActivity();
    var localStudentPreview = isLocalStudentPreview();
    ...
    session.innerHTML =
      ...
      '<div class="oc-session-guide"><span>1</span> ' + (localStudentPreview ? "활동을 시작해요" : "선생님이 시작해요") + ' <i></i><span>2</span> 학생이 풀어요 <i></i><span>3</span> ' + (localStudentPreview ? "직접 다음 문제로 가요" : "선생님이 다음 문제로 넘겨요") + '</div>' +
      (isTeacher() || localStudentPreview
        ? '<button class="oc-primary-action" type="button" data-action="start">' + (localStudentPreview ? "학생 미리보기 시작" : "수업 시작") + '</button>'
  ...
  function renderTeacherQuestion(item, answered) {
    ...
    return '<div class="oc-teacher-console">' +
      ...
      '<div class="oc-teacher-actions">' +
        ...
        '<button type="button" data-action="next"' + trackingAttributes(item, "activity-complete") + ' ' + (!answered ? "disabled" : "") + '>' + (state.index + 1 >= totalItems() ? "학습 마치기" : "다음 문제") + '</button>' +
      '</div></div>';
  }
  ...
  function renderStudentQuestion(item, answered, choicesReady) {
    if (answered) {
      ...
      return '<div class="oc-student-feedback ' + (correct ? "is-correct" : "is-wrong") + '">' + trainCelebration +
        ...
        (pacing() === "student" || isLocalStudentPreview()
          ? '<button class="oc-primary-action" type="button" data-action="next"' + trackingAttributes(item, "activity-complete") + '>' + (state.index + 1 >= totalItems() ? "학습 마치기" : "다음 문제") + '</button>'
    ...
    if (!choicesReady) {
      ...
      var lengthOptions = syllableCountOptions(item.parts.length).map(function (value) {
        return '<button type="button" data-action="guess-length" data-value="' + value + '"' + trackingAttributes(item, "answer", value, "length") + '>' + value + '자</button>';
      }).join("");
      ...
      var orderOptions = remaining.map(function (value) {
        return '<button type="button" class="oc-station-ticket" data-action="part" data-value="' + escapeHtml(value) + '" ' +
          trackingAttributes(item, "answer", value) + ' ' + (state.selected.length >= item.answer.length ? "disabled" : "") + '><span>탑승</span>' + escapeHtml(value) + '</button>';
      }).join("");
      answerArea = '<div class="oc-order-answer oc-order-train-game">' +
        ...
        '<button class="oc-check-answer" type="button" data-action="check-order"' + trackingAttributes(item, "activity-complete") + ' ' + (state.selected.length !== item.answer.length ? "disabled" : "") + '>확인하기</button></div></div>';
    } else if (state.activity === "anagram") {
      var parts = shuffle(item.parts).map(function (value) {
        return '<button type="button" data-action="part" data-value="' + escapeHtml(value) + '"' + trackingAttributes(item, "answer", value) + '>' + escapeHtml(value) + '</button>';
      }).join("");
      ...
      answerArea = '<div class="oc-answer-grid ' + (state.activity === "listen" ? "is-listen-options" : "") + '">' + choices.map(function (value) {
        return '<button type="button" data-action="answer" data-value="' + escapeHtml(value) + '"' + trackingAttributes(item, "answer", value) + '>' + escapeHtml(value) + '</button>';
      }).join("") + "</div>";
    }
