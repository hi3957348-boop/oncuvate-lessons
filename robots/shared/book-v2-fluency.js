(function () {
  ...
  function fluencyReportsKey() {
    var lessonId = document.documentElement.dataset.lessonId || "oc";
    return lessonId + ":fluency-reports-v2";
  }

  function isSelfStudyMode() {
    try { return !!(window.OcSession && window.OcSession.useMode === "self"); } catch (_) { return false; }
  }
  ...
  function showPresetToast() {
    var name = "";
    try { name = (window.OcSession && window.OcSession.childCode) || ""; } catch (_) {}
  ...
  window.addEventListener("oncuvate-session-change", function (event) {
    ...
    if (session.useMode !== "self") return;
  ...
  function learnerName() {
    try {
      if (window.OcSession && window.OcSession.childCode) return String(window.OcSession.childCode);
      return "학생";
    } catch (_) { return "학생"; }
  }
  ...
  function teacherCurrentResult(report) {
    return '<div class="oc-live-result-head"><span class="' + (report.confidence === "high" ? "is-high" : "") + '">새 결과</span><b>' + escapeHtml(report.childCode) + '</b></div>' +
  ...
  function reportCard(report) {
    ...
    return '<article class="oc-report-card"><header><div><strong>' + escapeHtml(report.childCode) + '</strong><span>' +
  ...
  function saveReports() {
    try { localStorage.setItem(fluencyReportsKey(), JSON.stringify(reports)); } catch (_) {}
  }

  function restoreReports() {
    if (!isTeacher()) return;
    try {
      reports = JSON.parse(localStorage.getItem(fluencyReportsKey()) || "[]");
