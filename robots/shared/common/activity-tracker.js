(function () {
  ...
  var lesson = {
    ...
    level: Number(root.dataset.lessonLevel || 0) || null,
    productGroup: root.dataset.productGroup || "",
    availableModes: String(root.dataset.availableModes || "").split(",").filter(Boolean)
  };
  if (!lesson.lessonId) return;

  var STATE_KEY = "oncuvate-progress:" + lesson.lessonId + ":" + lesson.version;
  var QUEUE_KEY = "oncuvate-progress-queue:v1";
  var SESSION_ID_KEY = "oncuvate-lesson-session:" + lesson.lessonId + ":" + lesson.version;
  ...
  var answerSelector = [
    ...
    '[data-action="answer"]', '[data-action="part"]', '[data-action="guess-length"]', '[data-action="find-word"]',
  ...
  function lessonSessionId(session) {
    if (session && (session.lessonSessionId || session.sessionId)) {
      return String(session.lessonSessionId || session.sessionId);
    }
    try {
      var saved = sessionStorage.getItem(SESSION_ID_KEY);
  ...
  function queueEvent(type, activityId, detail) {
    ...
    var sessionId = lessonSessionId(session);
    var studentId = String(session.studentId || "");
    var userId = String(session.userId || session.uid || "");
    ...
    var event = Object.assign({
      ...
      eventType: type,
      type: type,
      attemptNumber: type === "answer" ? Number(state.attempts || 0) : null,
      isCorrect: null,
      durationSeconds: summary().totalActivitySeconds,
      createdBy: userId || studentId || sessionId,
      status: /complete/.test(type) ? "completed" : "active"
    }, detail || {});
    if (typeof event.correct === "boolean") event.isCorrect = event.correct;
    var queue = getQueue();
    queue.push(event);
    setQueue(queue);
      ...
      var response = await fetch(apiUrl("/activity-progress"), {
        ...
        body: JSON.stringify({
          ...
          learner: {
            ...
            sessionId: lessonSessionId(session),
            studentCode: session.studentCode || "",
            roomCode: session.roomCode || "",
            role: session.role || "student"
          },
          serverInstructions: {
            resolveIdentityOnServer: true,
            useServerTimestampForCreatedAt: true,
            appendOnlyActivityEvents: true
          },
          events: batch,
  ...
  function createStatusArea() {
    ...
    panel.innerHTML =
      ...
      '</span>' +
      '<button type="button" class="oc-lesson-complete" data-track="lesson-complete" data-activity-id="' + lesson.lessonId + ':lesson:complete" aria-label="수업을 완료하고 학습 결과 확인">수업 완료</button>';
  ...
  document.addEventListener("click", function (event) {
    ...
    if (element && element.dataset.trackManual !== "true") onTrackedActivation(element);
  }, true);

  document.addEventListener("change", function (event) {
    var element = event.target.closest('[data-track="answer"]');
    if (element && element.dataset.trackManual !== "true" && !/^(button|a)$/i.test(element.tagName)) onTrackedActivation(element);
  }, true);
  ...
  window.addEventListener("oncuvate:track", function (event) {
    ...
    if (detail.type === "answer") {
      ...
      addCompleted(activityId);
      if (detail.correct) addCorrect(activityId);
    }
