...
(function(){
  ...
  if(R.role==='coach'){
    ...
    window.addEventListener('oncuvate:log',event=>{
      ...
      const body={access_key:key,subject:`[Oncuvate Pilot] Titanic ${d.sessionNo||'?'} · coach note · ${d.childId||''}`,from_name:'Oncuvate Pilot Coach Note',lesson_id:'titanic-voyage',session_no:String(d.sessionNo||''),participant_code:String(d.childId||''),room:String(R.room||''),note_text:String(d.text||''),note_json:JSON.stringify(d),botcheck:''};
  ...
  const KEY='oncuvate.pilot.titanic-voyage.log.'+sourceFile+'.v1';
  const OUTBOX='oncuvate.pilot.titanic-voyage.outbox.v1';
  const ENDPOINT='https://api.web3forms.com/submit';
  const MAX_EVENTS=1200;
  let record=load()||fresh();
  let dirty=false,timer=0,busy=false;

  function fresh(){return{schema:'oncuvate.pilot-log.v1',lessonId:'titanic-voyage',sourceFile,sessionNo:Number(R.session)||0,child:String(R.child||''),room:String(R.room||''),classroomCode:String(CONFIG.classroomCode||'TITANIC-VOYAGE-PILOT'),sessionId:'sp-'+Date.now().toString(36)+'-'+Math.random().toString(36).slice(2,8),startedAt:new Date().toISOString(),events:[],tracks:[],completed:false,completedAt:null,lastSubmittedAt:null}}
