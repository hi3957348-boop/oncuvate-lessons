(function(){
  ...
  const caseVocab=window.OncuvateCaseVocab?.create({words:C.words,onOpen:function(word){if(!S.vocabOpened.includes(word))S.vocabOpened.push(word);signals.log('word-open',{activityId:screenActivity[S.screen]||S.screen,word:word});save();updateChrome()}});
  ...
  function renderSystemGame(){
    ...
    const planHtml='<section class="side-panel"><h3>MY 3-STEP PLAN</h3><p>Choose the order before touching the controls.</p><div class="mission-order">'+[0,1,2].map((_,i)=>'<div>STEP '+(i+1)+' · '+(S.plan[i]?esc(C.game.items.find(x=>x.id===S.plan[i]).name):'Choose a system')+'</div>').join('')+'</div>'+(remaining.length?'<div class="control-row">'+remaining.map(x=>'<button data-plan="'+x.id+'" type="button">'+x.name+'</button>').join('')+'</div>':'<p class="feedback success">Plan ready. Repair one system at a time.</p>')+(S.plan.length?'<button class="quiet" id="clearPlan" type="button">계획 다시 세우기</button>':'')+'</section>';
