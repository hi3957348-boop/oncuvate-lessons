# 젤리 우주탐정 · 파일럿 사본 (space-observatory-pilot)

이 폴더는 `tools/build-pilot.py space-observatory`가 **납품본 `space-observatory`에서 생성**한다. 직접 고치지 말고 납품본을 고친 뒤 다시 빌드한다.
...
- `pilot-firebase-core.js` — 파일럿 Firebase 방(`space-observatory-pilot/rooms/<방>`), 3시간 뒤 닫힘.
- `pilot-config.js` / `pilot-log-relay.js` — `oncuvate:log`·`data-track`을 기기에 모아 Web3Forms로 묶어 보낸다(정식에서는 플랫폼 트래커가 한다).

## 열기
1. `pilot-entry.html`을 연다(로컬 확인은 launch.json `space-observatory-pilot`, 포트 8986).
