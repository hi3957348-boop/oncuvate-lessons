...
const ROOT="space-observatory-pilot/rooms";
const ROOM_LIFETIME_MS=3*60*60*1000;
const app=getApps().find(item=>item.name==="space-observatory-pilot")||initializeApp(firebaseConfig,"space-observatory-pilot");
...
export async function createRoom(value,sessionFile){
  ...
  const result=await runTransaction(metaRef(room),current=>{if(current&&Number(current.expiresAt||0)>now)return;return{lessonId:"space-observatory-pilot",sessionFile:file,status:"open",createdAt:now,expiresAt:now+ROOM_LIFETIME_MS,updatedAt:now}},{applyLocally:false});
